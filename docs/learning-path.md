...
4. **Source sweep — [core — execution engine in the source map](reference/spark-source-map/sweeps/core-execution-engine.md)** — the lock as `TaskIdentifier(stageAttempt, taskAttempt)`, why a request arriving after `stageEnd` is always denied, how a failed attempt is permanently barred while its lock is released, and the single call site the whole mechanism guards
