...
!!! warning "Deprecated — use [Learning Path v2](learning-path-v2.md)"
    This page is **no longer the path you study from**. [Learning path v2](learning-path-v2.md) replaced it: 185 topics in strands, every capability area in the [feature history](reference/spark-feature-history/index.md) audited row by row against Spark 4.2.0, and a stated position on what it deliberately does not cover.

    This page is kept **frozen, as a detail store**. Its value is the long per-topic `!!! info` / `!!! warning` blocks recording specific source findings, and the changelog above, which is a record of how the source map was built. Its topic codes are v1 codes — the [v1 → v2 map](learning-path-v2.md#v1-v2-code-map) crosswalks them. Nothing here is maintained: version claims, topic counts and status markers on this page are as of 2026-08-09 and are not being updated.
...
!!! warning "Marked 🔄 — for Ch03 only now"
    ...
    Chapter 02 was **rewritten against Spark 4.2.0 on 2026-08-11** and is now clear. The rewrite corrected both errors the 2026-08-10 pass found — the three-stage word-count walkthrough describing a `rangepartitioning` shuffle that `TakeOrderedAndProjectExec` means Spark never plans, and "one action = one job" stated as an invariant — and closed the nine outstanding gaps listed below. Every stage, job and partition count in it was measured against a running 4.2.0 instance rather than inferred from the transformation chain.
