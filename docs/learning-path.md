...
4. **Related:** **A36** for the offset/commit-log protocol the state store checkpoints alongside, **E28** for changing the partition count, **I6** for the memory model the HDFS-backed provider competes with
5. **Related config family:** the whole `spark.sql.streaming.stateStore.*` namespace — read `StateStoreConf.scala` rather than the docs page; it is the only complete list
