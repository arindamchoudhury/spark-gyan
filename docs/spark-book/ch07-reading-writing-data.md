...
> *Learning-path topic: B10 (Beginner)*
...
!!! warning "🔄 Needs revisiting — Spark 4.2.0 source trace (flagged 2026-07-18)"
    ...
    **Added by the sql/core — datasources sweep (2026-08-04):** three more, one of which is a correction. `spark.sql.files.ignoreCorruptFiles` does **not** skip a corrupt file — it marks the partition finished at the point of failure and keeps the rows read so far, so the job succeeds with silently truncated data; any wording that presents it as "skip the bad file" is wrong. The packing is *Next Fit Decreasing* and charges `openCostInBytes` (4MB) per file on top of its real length, so that surcharge — not `maxPartitionBytes` — is the knob for the small-files case. And before any of this, the **driver must list every file**: on a large partitioned table that listing is a separate Spark job with no SQL node attached, and it is frequently why a query takes minutes to start. Detail in the [sql/core — datasources sweep](../reference/spark-source-map/sweeps/sql-core-datasources.md), and see new topics I30, I31 and E32.
    ...
    **`DROPMALFORMED` discards rows silently.** The parse modes are not covered. `DROPMALFORMED` drops bad records with no count and no log; `PERMISSIVE` (the default) emits them as all-null rows that are indistinguishable from ordinary missing data unless you select `columnNameOfCorruptRecord`.
    ...
    Also missing: `maxRecordsPerFile` as the output-file-size lever, *why* inference is costly (it reads the data twice), ORC defaulting to `zstd` while Parquet uses `snappy`, `ignoreCorruptFiles`/`ignoreMissingFiles`, JSON writes dropping null fields by default, predicate pushdown being best-effort per filter rather than a property of the format, driver-side file listing as the cause of long pre-task stalls, `PartitionedFile` being a slice rather than a whole file, `DataFrameWriterV2`, and `bucketBy` being silently ineffective outside table writes. Full list in the [B10 source trace](../reference/spark-source-map/topics/b4.md).

!!! note "Also flagged for B10 (added to the path 2026-07-21) — Python Data Sources"
    A 4.2.0 feature this chapter should gain on revisit: **Python Data Sources** — a custom reader/writer implemented entirely in Python (`pyspark.sql.datasource.DataSource` + `spark.dataSource.register(...)`), plugging into the same `spark.read.format(...)` / `df.write.format(...)` surface, covering batch and streaming, read and write ([SPARK-55304] streaming reader; [SPARK-55161] connector profiling). It is the pure-Python cousin of DSv2. See the B10 callout in the learning path.
