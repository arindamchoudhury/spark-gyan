...
!!! note "📌 Topics deferred here from Chapters 1 and 2"
    ...
    - **Physical plan selection** — the `SparkPlanner` generates multiple candidate physical plans for each logical operator (e.g. SortMergeJoin, BroadcastHashJoin, ShuffledHashJoin for a join node) and selects the lowest-cost candidate; see also Chapter 24 (A3) for execution-level join differences
