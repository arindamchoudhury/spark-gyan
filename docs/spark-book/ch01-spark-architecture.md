...
**MapOutputTracker** — a directory service for shuffle data. When a map stage completes, each task registers the location of its output partitions with the driver-side tracker. When a downstream stage starts, its tasks query the tracker to find which executor holds each input partition before opening fetch connections. Without it, shuffle reads would have no way to locate their data. The internal caching, invalidation, and consistency mechanics are covered in **Chapter 30 (E1 — Spark Internals)**.

**BlockManager** — runs on every executor and on the driver. It manages two things: cached data (RDD/DataFrame partitions and broadcast variables), held in memory or spilled to local disk; and serving as the network interface through which remote tasks fetch shuffle blocks from this executor. Shuffle data itself is written directly to disk by the shuffle writer and bypasses BlockManager's own storage — BlockManager only serves it over the network on request. Uncached input data bypasses BlockManager entirely and is read directly from the source (HDFS, S3, etc.). The internal storage layers, block addressing, and network transfer mechanics are covered in **Chapter 30 (E1 — Spark Internals)**.
