...
To counter this, Spark uses **lineage**. Every RDD records how it was derived — which parent RDD it came from and which transformation produced it. This chain of derivations reaches all the way back to the original source data, which is durably stored in HDFS or S3. If an executor crashes and its in-memory partitions are lost, Spark does not restart the whole job. It walks the lineage graph, identifies the minimal set of source partitions needed to reconstruct the lost ones, and recomputes only those partitions — on a surviving executor. The rest of the job continues uninterrupted. Lineage is what makes it safe to keep intermediate results only in memory: you never need a replica, because you can always rebuild.
