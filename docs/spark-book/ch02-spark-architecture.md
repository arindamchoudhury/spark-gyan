...
The four phases run in sequence when `withAction()` accesses `executedPlan` — the output of each becomes the input of the next. When `PrepareForExecution` completes, the `executedPlan` is final. Spark then walks the operator tree to produce an `RDD[InternalRow]` — the entire query expressed as a chain of RDD objects, each recording how it was derived from its parent. Each RDD covers the full dataset — a single RDD object represents all partitions; the partition count is just metadata inside it. With whole-stage codegen enabled (the default), multiple consecutive operators are fused by `WholeStageCodegenExec` into a single `mapPartitionsWithIndex` call, so one RDD is produced per fused group — not one per individual operator. Without codegen, each operator's `doExecute()` produces its own wrapping RDD. Narrow transformations (`filter`, `select`) form an unbroken chain that runs in a single pass; wide operators (`groupBy`, `join`) cause `ShuffleExchangeExec` to embed a `ShuffleDependency` into the RDD chain — marking where a stage boundary is needed. This graph of RDD objects and their dependencies is the lineage. It is then handed to `SparkContext.runJob()`, which immediately delegates to `dagScheduler.runJob()`. The DAGScheduler walks this lineage, finds those `ShuffleDependency` objects, and cuts the stage boundaries there — the operators mark the boundary; the DAGScheduler enforces it.
...
- `PrepareForExecution` turns `sparkPlan` into `executedPlan` by applying preparation rules: `EnsureRequirements` (inserts `ShuffleExchangeExec` at wide-dependency boundaries) and `CollapseCodegenStages` (groups consecutive codegen-capable operators and wraps each group in a `WholeStageCodegenExec` node). This is a **structural planning decision** — no compilation happens here.
- `executedPlan.execute()` calls `doCodeGen()` on each `WholeStageCodegenExec` node, producing a **Java source string** for the fused operator group. A **validation compile** then runs immediately on the driver (`CodeGenerator.compile`) — if it fails, the node falls back to interpreted execution. The source string is packaged into `WholeStageCodegenEvaluatorFactory` and travels to executors with the task. The `RDD[InternalRow]` is what `execute()` returns.

**JVM bytecode compilation is executor-side.** The driver generates and validates Java *source*; executors compile it. When a task starts, `WholeStageCodegenEvaluatorFactory.createEvaluator()` calls `CodeGenerator.compile(source)` — Janino compiles the source to JVM bytecode on the executor, cached per executor JVM. The fused operator chain then runs as a tight native bytecode loop with no virtual method calls between operators. That — together with built-in functions (`F.lower()`, `F.sum()`, …) running entirely in the JVM — is why the DataFrame API runs fast regardless of what the Python layer above looks like. Source-verified v4.1.2: `WholeStageCodegenExec.scala` L731–734 (driver: generate + validate), `WholeStageCodegenEvaluatorFactory.scala` L45 (executor: compile).
...
**What "pipelined execution" means.** Step 2 above — `split → lower → filter` — is not three separate passes over the partition data. It is a single iterator-based pass: each row flows through all three operations before the next row is processed. There is no intermediate materialization between operators within a stage. When Tungsten whole-stage codegen is active, `createEvaluator()` compiles the Java source (generated on the driver in Stage 1) to JVM bytecode here on the executor via Janino — all fused operators run as a single tight loop with no virtual method calls between them. This is the operational meaning of "pipelined": one pass, one loop, no intermediate buffers.
