...
> **Created:** 2026-08-09 — first full re-carve of the path since it was written, and rebuilt since: 186 topics in strands, every capability area of the [feature history](reference/spark-feature-history/index.md) audited row by row against Spark 4.2.0. The three most recent passes are below; the full dated record of every pass — including the ones that corrected a wrong claim rather than adding a topic — is the [changelog](learning-path-v2-changelog.md).
>
...
flowchart TD
    ...
    A["<b>Advanced</b> — make it fast, make it stream<br/>61 topics · 97–134 hrs"]
...
|---|---|
| Open table formats (Iceberg increasingly the default; Delta where Databricks is in play) | I37 Delta, I38 Iceberg and interop, A39 and A61 keeping each one maintained |
...
**Estimated time:** 97–134 hrs · **61 topics**
...
flowchart LR
    ...
    subgraph ADV["Advanced · 61 · 97–134 hrs"]
...
Every v1 topic appears exactly once. The table below lists the seven topics that were new when v2 was written; the thirty-five added since — thirty-four from the feature-history audits (**I44**–**I53**, **A46**–**A60**, **E52**–**E60**) and **A61** from the lakehouse-coverage review — carry a **New topic** line in place of a v1 code and are not repeated here.
...
- Local table-format checkouts — `C:\opt\learn\spark\repos\delta` (tag `v4.3.1`) behind **I37** and **A39**, and `C:\opt\learn\spark\repos\iceberg` (tag `apache-iceberg-1.11.0`) behind **I38** and **A61**. Every default those four topics quote is read from these trees rather than from either project's documentation, which states some of them loosely and others not at all
