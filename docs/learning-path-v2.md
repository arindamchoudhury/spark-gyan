...
**What** — the three ways a DataFrame's partitioning changes and what each costs. **`repartition(n)`** and **`repartition(cols…)`** shuffle: the first round-robins to exactly `n` partitions, the second hash-partitions by the columns so equal keys land together. **`repartitionByRange(cols…)`** samples the data to build range boundaries, which is what makes a sorted write produce non-overlapping files. **`coalesce(n)`** does **not** shuffle — it merges existing partitions into fewer, which is why it can only reduce and why it propagates *upward*: `coalesce(1)` before a wide transformation makes the whole upstream stage run with one task. The default width of every shuffle is `spark.sql.shuffle.partitions`, **200** since 1.1.0 and unchanged since, which is a number chosen for a cluster nobody runs any more. On the write side, `partitionBy` is a different thing entirely — directory layout, read by partition pruning (**I30**) — as is `bucketBy` (**I32**).

**Why** — because the partition count is the task count, and both of the ways to get it wrong are quiet. Too few and you leave the cluster idle while a handful of tasks chew through oversized partitions; too many and you pay scheduling overhead per task plus a shuffle with `partitions²` blocks — the small-file and small-block problem that **A27** exists to mitigate. The `coalesce` trap is the sharpest of the three because it looks local: it does not move data, so it is cheap, and the price is paid by a stage you did not edit. And note what AQE (**A11**) changed: with adaptive execution on, the 200 is a *starting* number that coalescing may reduce at runtime, so tuning it by hand matters less than it used to — but only for shuffles AQE can see, which is why **A19** on what AQE actually measures belongs next to this.
...
**Why** — the contrast with Delta (**I37**, **E33**) is the point, and it is not "which is better". Iceberg's per-manifest statistics and hidden partitioning mean planning can prune files without listing directories, which is why it scales to very wide tables and why partition evolution is possible at all; Delta's single ordered log gives simpler reasoning about concurrency and a cheaper small-table path. Practically, the decisions you inherit differ: an Iceberg reader must respect **field ids**, so a rename is safe where a positional format would corrupt (**E32**); and hidden partitioning means a query with no partition predicate can still prune, which changes how you write filters. The interop layers — Delta UniForm, the Iceberg REST catalog — exist because most organisations end up with both, and the honest answer to "which format" is usually "which catalog do your engines already agree on".
