...
Then the one that destroys data. **`remove_orphan_files` is the only maintenance operation that does not work from metadata alone** — it lists the filesystem under the table location and diffs that against the table's known file set, so anything that makes the two lists spell the same file differently makes live data look orphaned. The classic case is scheme drift: the catalog records `s3://bucket/…` and the listing yields `s3a://bucket/…`. `DeleteOrphanFilesSparkAction` defends with `prefixMismatchMode` defaulting to **`ERROR`** and an `equalSchemes` map that pre-equates only `s3n,s3a → s3`; the way people delete their own table is setting that mode to `DELETE` to make the error stop. Two more guards worth knowing before you need them: the *procedure* refuses an `older_than` more recent than **24 hours** — the message names concurrent operations as the reason — while the *Action API* has no such check, and the default is **now − 3 days** either way; and `gc.enabled` (default **`true`**) is a table property that disables the destructive actions entirely, which is what you set on a table whose files another engine's snapshots still reference.
...
**New topic** · no v1 code · sourced from the lakehouse-coverage review that produced **A61** and **A62**, which between them leave one question open: every maintenance operation those topics teach is a job somebody must schedule, size and watch, and this page's only answers were **E47** (write the cron yourself) or a vendor. Amoro is the open-source system that does it as a service, and it is essentially the only one, so what it automates is worth knowing whether or not you deploy it

**What** — verified against the local checkout at `C:/opt/learn/spark/repos/amoro`, tag `v0.8.1-incubating`. Amoro is a **management system**: not a format, not an engine, and that is the thing to fix in your head first. **AMS**, the Amoro Management Service, is the server — it holds a unified catalog across engines, optionally fronting an existing Hive Metastore, continuously reads table metadata, and decides what maintenance is due. The work is carried out by **optimizers**: long-lived processes AMS dispatches tasks to, shipped in four runtimes (`amoro-optimizer-flink`, `-spark`, `-standalone`, and an `external` container where you run them yourself). Optimizers are pooled into **groups**, and each table names the group it draws on (`self-optimizing.group`, default `default`). Optimizing comes in three types — `MINOR`, `MAJOR` and `FULL` — differing in how much they rewrite. Format coverage is wider than any single format's own tooling: `amoro-format-iceberg`, `amoro-format-paimon`, `amoro-format-hudi`, and `amoro-format-mixed` for Amoro's own Mixed-Iceberg and Mixed-Hive.
