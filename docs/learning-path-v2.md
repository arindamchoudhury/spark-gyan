...
> **Created:** 2026-08-09 — first full re-carve of the path since it was written, and rebuilt since: 188 topics in strands, every capability area of the [feature history](reference/spark-feature-history/index.md) audited row by row against Spark 4.2.0. The three most recent passes are below; the full dated record of every pass — including the ones that corrected a wrong claim rather than adding a topic — is the [changelog](learning-path-v2-changelog.md).
>
> **Updated:** 2026-08-14 — reviewed the lakehouse strand for coverage rather than auditing a feature-history area, after a look at the managed table-maintenance products (Amoro, LakeOps, and the vendor equivalents) raised the question of what this page says about the work they automate. It said a great deal about Delta's — **A39** owns `OPTIMIZE`, `ZORDER`, liquid clustering, deletion vectors and `VACUUM` — and nothing about Iceberg's: **I38** taught the manifest tree and hidden partitioning, then stopped at the read path, and neither `rewrite_data_files` nor `expire_snapshots` appeared anywhere on the page. Added **A61** for Iceberg's upkeep as a whole: the procedures (`CALL <catalog>.system.…` rather than SQL commands), the defaults that decide what they do — `history.expire.max-snapshot-age-ms` **5 days** and `min-snapshots-to-keep` **1**, neither of which runs until you call the procedure; `write.metadata.previous-versions-max` **100** capped by a `delete-after-commit.enabled` that is **`false`**; `min-input-files` **5**; `partial-progress.enabled` **`false`**, so a failed multi-hour compaction commits nothing — and the one operation that can delete live data, `remove_orphan_files`, which diffs a filesystem listing against table metadata and is guarded by `prefixMismatchMode` defaulting to **`ERROR`**, a 24-hour minimum the procedure enforces and the Action API does not, and the `gc.enabled` kill switch. Every figure read from the local Iceberg checkout at tag `apache-iceberg-1.11.0`, which also settled a version fact worth stating: it ships Spark modules for 3.4–4.1 and **none for 4.2**. The same pass then added **A62** for **Apache Paimon**, which the Amoro reading turned up as a first-class format beside Iceberg and which this page had never mentioned: the two formats it did teach share one design — immutable files plus a metadata log — and Paimon does not, being an **LSM tree** with buckets, sorted runs, a **merge engine** (`deduplicate` by default, with `partial-update`'s non-null rule as the trap) and a **changelog producer** (`none` by default). Its headline fact is the exact inverse of **A61**'s: expiry is not something you call but something the commit does — `TableCommitImpl` runs it post-commit with `snapshot.expire.execution-mode` defaulting to **`SYNC`** — against `snapshot.num-retained.min` **10** and `snapshot.time-retained` of **one hour**, so time travel there is a streaming-restart window rather than a history, and tags, not retention, are how anything is kept. Verified against a new local checkout at tag `release-2.0.0`, which ships Spark modules for 3.2–4.1 and, like Iceberg, **none for 4.2**. Finally **A63** for **Apache Amoro** itself, closing the question the other two leave open — who runs the maintenance — since the page's only prior answers were **E47** or a vendor. Amoro is a management system rather than a format or an engine: **AMS** holds a catalog and decides what is due, **optimizers** in three shipped runtimes execute it, and a table buys work with a **quota** (`self-optimizing.quota`, **0.1**) instead of owning a schedule. `self-optimizing.enabled` is **`true`**, the exact inverse of Iceberg's nothing-runs default — but `full.trigger.interval` is **`-1`**, so the thorough operation is off, which is the same shape as **A61**'s `partial-progress` and **A62**'s one-hour retention. Verified at tag `v0.8.1-incubating`, which also settles the adoption question honestly: it builds against Iceberg **1.6.1** and Paimon **0.9.0**, pins Spark **3.3.3** for its Spark optimizer, and ships its Mixed-format Spark connector for **3.2/3.3/3.5 with nothing on the 4.x line** — so for Iceberg and Paimon it manages your tables from outside as a service and never becomes a Spark dependency, and its own connector is unavailable on the Spark this path targets.
>
> **Updated:** 2026-08-14 — verification pass, first over the three new lakehouse topics and then over the older ones, on a class of claim the page had never checked separately: not the figures, which hold up, but the **prose written around them at the same confidence**. Eight corrections in **A61**–**A63**, the sharpest being that A61 named `s3://` versus `s3a://` as the orphan-file danger when `EQUAL_SCHEMES_DEFAULT` pre-equates exactly that pair — the unguarded case is an *authority* mismatch, since `equalAuthorities` defaults to an empty map — and a milestone that asked the reader to reproduce an error that therefore cannot fire. Also: 20 procedures rather than 22 (count the registry, not the directory); Puffin sketches reaching Spark through DSv2 `SupportsReportStatistics` behind a `spark.sql.cbo.enabled` that defaults **false**, not through **A10**'s `ANALYZE TABLE` path; Paimon's sequence fallback being `MergeTreeWriter` counting from the bucket's `maxSequenceNumber + 1` rather than arrival order; and Amoro shipping three optimizer runtimes, not four. Then four **complete** mechanical checks across all 188 topics — 188 cross-references resolve, 216 config names resolve, 125 source anchors exist, and of 384 cited types 335 are declared in Spark with the rest correctly belonging to other trees. One real error fell out: **A51** listed `MetadataOnlyTable` (SPARK-52729) among 4.2.0's v2-catalog additions, but no such type exists in the OSS tree and the ticket appears once, in a `DataSourceV2Strategy` comment reading "Tracked for follow-up real handlers" above arms that make `REFRESH TABLE` and `ANALYZE TABLE` throw `UNSUPPORTED_FEATURE.TABLE_OPERATION`. The change made a gap *legible*, not closed — indistinguishable from a feature in a release note and opposite in the source, which is the second instance after **B12**'s `ai_complete`. Also corrected the page's own bookkeeping, wrong before this session: 42 topics carry a **New topic** line, not 37 or 41, and the crosswalk's enumeration had silently omitted **B12**, **I5**, **I6**, **I13** and **I14**.
>
> **Updated:** 2026-08-14 — behaviour audit of the tuning spine, **A1**–**A24** read end to end against the classes they name rather than checked for the existence of those names, then the same lens turned on the milestones and observability claims of the other 164 topics. Three errors, all in one seam — **what the page says you would see**, never what the engine computes. **A12** told readers a re-plan visible in the `Plan changed:` log may still be discarded; `logOnLevel` sits *inside* the adoption branch, so that line means the plan was **kept** and a rejected re-plan prints nothing at all — and its milestone asked for a constant `CostEvaluator` to stop adoption, which the `newCost == origCost && plans differ` clause defeats. It also described only one of `SimpleCostEvaluator`'s two modes and sent readers to write a class when `spark.sql.adaptive.forceOptimizeSkewedJoin` was the fix. **A9** presented both of `UNION ALL`'s 4.x wins as landed, but `spark.sql.codegen.wholeStage.union.enabled` is 4.2.0 and defaults **`false`**, so the first of its eight disqualifiers fires out of the box — and it denied an observable that exists, since `UnionExec.metrics` registers `numOutputRows` only when fusion will run. **A1** opened a milestone with `EXPLAIN(true, true)`, a form valid in neither language: `DataFrame.explain` raises `CANNOT_SET_TOGETHER` when `extended` and `mode` are both set. Everything derived or countable held: `LOG_BASE = 1.1` giving ~36 GB saturation in **A19**, exactly eight union disqualifiers, exactly nine segment-tree aggregates in **A22**, `satisfies`'s `numPartitions` precondition in **A4**, the 1 → 4 → 16 limit escalation in **A7**. Refined **A13**'s row-count claim to the asymmetric contract the source states (zero is exact by construction; positive values may be over-estimated) and gave **A16**'s no-spill assertion its evidence, `def spill(...): Long = 0L`. The sweep outside the spine found **no** errors, which retires the theory that observability is a page-wide weakness: **A1**, **A9** and **A12** were a local cluster. **E3** gained the one thing missing — its "reported at INFO by default" is load-bearing, because `spark.storage.exceptionOnPinLeak` turns the same branch into a thrown error.
>
...
| Spark docs | [Apache Spark 4.2.0 documentation](https://spark.apache.org/docs/latest/) | Official docs | Everything factual — cited per topic rather than by key |
| Delta / Iceberg docs | [docs.delta.io](https://docs.delta.io/latest/) · [iceberg.apache.org](https://iceberg.apache.org/docs/latest/) | Official docs | Table formats — cited per topic rather than by key |
...
flowchart TD
    ...
    A["<b>Advanced</b> — make it fast, make it stream<br/>63 topics · 97–134 hrs"]
...
| **Advanced** | How a query is compiled · Statistics and adaptive execution · Joins, aggregation and windows at scale · Reliability of a running job · The file boundary · Streaming · Pipelines · Engineering practice · Pushdown and the write path · Stragglers · The connector API itself · Machine learning · Shuffle mechanics · Arbitrary state, and seeing into it · Keeping a table maintained · The streaming-first table format · Automating the lakehouse |
...
|---|---|
| Open table formats (Iceberg increasingly the default; Delta where Databricks is in play) | I37 Delta, I38 Iceberg and interop, A39 and A61 keeping each one maintained, A62 Paimon for the streaming-upsert case neither answers well |
...
**What** — the family that hands your Python function a whole partition or a whole group rather than a column: `DataFrame.mapInPandas` and `DataFrame.mapInArrow` over partitions, `GroupedData.applyInPandas` and `GroupedData.applyInArrow` over groups, and cogrouped `applyInPandas` / `applyInArrow` over two grouped frames aligned on a key. These are *not* UDFs — they do not return a column, they return a new DataFrame with a schema you declare, and that is what lets them change the row count and what makes the schema argument is mandatory. Verified at tag `v4.2.0`, the signature is `mapInArrow(func, schema, barrier=False, profile=None)`: `barrier` runs the stage in barrier mode, and `profile` attaches a `ResourceProfile`, so this family is the practical entry point to two features that are otherwise Expert-level. 4.1 added the iterator forms — `Iterator[pandas.DataFrame]` for `applyInPandas` and iterator-of-`RecordBatch` for `applyInArrow` — so a group larger than memory no longer has to materialise at once, plus `maxRecordsPerOutputBatch` / `maxBytesPerOutputBatch` to bound what is handed downstream.
...
**What** — the three ways a DataFrame's partitioning changes and what each costs. **`repartition(n)`** and **`repartition(cols…)`** shuffle: the first round-robins to exactly `n` partitions, the second hash-partitions by the columns so equal keys land together. **`repartitionByRange(cols…)`** samples the data to build range boundaries, which is what makes a sorted write produce non-overlapping files. **`coalesce(n)`** does **not** shuffle — it merges existing partitions into fewer, so it can only reduce — and it propagates *upward*: `coalesce(1)` before a wide transformation makes the whole upstream stage run with one task. The default width of every shuffle is `spark.sql.shuffle.partitions`, **200** since 1.1.0 and unchanged since, which is a number chosen for a cluster nobody runs any more. On the write side, `partitionBy` is a different thing entirely — directory layout, read by partition pruning (**I30**) — as is `bucketBy` (**I32**).

**Why** — because the partition count is the task count, and both of the ways to get it wrong are quiet. Too few and you leave the cluster idle while a handful of tasks chew through oversized partitions; too many and you pay scheduling overhead per task plus a shuffle with `partitions²` blocks — the small-file and small-block problem that **A27** exists to mitigate. The `coalesce` trap is the sharpest of the three because it looks local: it does not move data, so it is cheap, and the price is paid by a stage you did not edit. And note what AQE (**A11**) changed: with adaptive execution on, the 200 is a *starting* number that coalescing may reduce at runtime, so tuning it by hand matters less than it used to — but only for shuffles AQE can see, so read **A19** on what AQE actually measures alongside it.
...
**What** — a Delta table is a directory of ordinary Parquet files plus a `_delta_log/` subdirectory, and everything Delta does follows from that log. Each commit is a JSON file named with a **zero-padded 20-digit version** (`_delta_log/00000000000000000042.json`) holding *actions* rather than data: which files were added, which removed, what the schema now is. A reader builds the table state by replaying the log, so a read is a snapshot at one version — which is what gives you **time travel** (`versionAsOf`, `timestampAsOf`) and what makes a write atomic: the data files are written first and are invisible until the commit that names them lands. Around that: schema enforcement on write with `mergeSchema` / `overwriteSchema` as the deliberate escapes, `DESCRIBE HISTORY` to read the log as a table, and table properties under the `delta.` prefix.

**Why** — three defaults, verified in `DeltaConfig.scala` at Delta **4.3.1**, decide how long your history actually lasts, and they are not the same number. `delta.logRetentionDuration` is **30 days** — how long *commit files* are kept. `delta.deletedFileRetentionDuration` is **1 week** — how long *data files a commit removed* are kept before `VACUUM` may delete them. `delta.checkpointInterval` is **10** — every tenth commit is compacted into a `.checkpoint.parquet` so a reader need not replay from zero. The consequence people meet in production: **time travel is bounded by the shorter of the two retentions, not by the log**. A version whose commit file still exists but whose data files were vacuumed is not readable, and the error names a missing file rather than a retention policy. Running `VACUUM` with a short retention to reclaim space silently shortens how far back you can query — which is fine if you decided it, and a data-loss incident if you did not.

**Learn** — DLDG Ch 1–3; DLUR Ch 1–4 · docs: [Delta Lake documentation](https://docs.delta.io/latest/index.html) — the quickstart, then table batch reads and writes; [table properties reference](https://docs.delta.io/latest/table-properties.html) for the `delta.` prefix · source: the local checkout at `C:/opt/learn/spark/repos/delta` (tag `v4.3.1`) — `PROTOCOL.md`'s "File Types" section shows the on-disk layout in ten lines, and `spark/src/main/scala/org/apache/spark/sql/delta/DeltaConfig.scala` carries every default quoted above · related: **I36** (the Parquet underneath), **I38** (Iceberg, the other answer), **A39** (the operations), **E33** (the protocol), **I39**

**Milestone** — create a Delta table, write to it three times, and list `_delta_log/` — name every file you see and say which commit each belongs to. Read the table `versionAsOf` its first version and show the old data. Then write eleven commits and find the checkpoint; say what a reader does differently once it exists. Change the schema of an incoming DataFrame and observe the write being rejected, then repeat with `mergeSchema` and say what the log recorded. Finally, `VACUUM` with a retention shorter than the default, try the time-travel read again, and explain the error you get in terms of the two retention properties rather than in terms of Delta being broken.

#### ⬜ I38 — Apache Iceberg and Table-Format Interoperability

`v1: I11`

**What** — Apache Iceberg as the other table format, verified against the local checkout at `C:/opt/learn/spark/repos/iceberg` (**1.11.0**). The metadata tree is the thing to hold: a catalog pointer → a **metadata file** (schema, partition specs, snapshot list) → a **manifest list** per snapshot → **manifests** listing data files with per-column statistics. Reads resolve a snapshot; writes add one. From that shape come **hidden partitioning** (the table stores a partition *transform* — `days(ts)`, `bucket(16, id)` — so queries filter on the raw column and Spark prunes anyway), **partition evolution** (change the spec without rewriting old data, because each manifest records the spec it was written under), full **schema evolution** by field id rather than by name or position, and time travel by snapshot. The **REST catalog** specification is what makes the catalog itself pluggable across engines.

**Why** — the contrast with Delta (**I37**, **E33**) is the point, and it is not "which is better". Iceberg's per-manifest statistics and hidden partitioning mean planning can prune files without listing directories, which is what makes it scale to very wide tables and what makes partition evolution possible at all; Delta's single ordered log gives simpler reasoning about concurrency and a cheaper small-table path. Practically, the decisions you inherit differ: an Iceberg reader must respect **field ids**, so a rename is safe where a positional format would corrupt (**E32**); and hidden partitioning means a query with no partition predicate can still prune, which changes how you write filters. The interop layers — Delta UniForm, the Iceberg REST catalog — exist because most organisations end up with both, and the practical answer to "which format" is usually "which catalog do your engines already agree on".

**Learn** — Iceberg-DG Ch 2–3; DLDG Ch 1 for the contrast · docs: [Iceberg Spark getting started](https://iceberg.apache.org/docs/latest/spark-getting-started/), [multi-engine support](https://iceberg.apache.org/multi-engine-support/) · source: [trace I11](reference/spark-source-map/topics/i11.md) · related: **A17** (storage-partitioned joins), **E29** (catalogs), **A61** (keeping the table you just made healthy), **A62** (the third format, which shares none of this design)
...
**What** — Bronze → Silver → Gold as a *contract* between layers rather than three folder names. **Bronze** is the landing zone: raw records appended as they arrived, schema as loose as the source, nothing dropped, so it can be replayed. **Silver** is where the contract tightens — typed columns, enforced schema, deduplicated by a business key, late and malformed records handled deliberately (**I29**), joins to reference data resolved. **Gold** is shaped for consumption: aggregates, denormalised marts, the tables a BI tool or a model reads. The Spark-side mechanics are ones this path already covers: schema enforcement and evolution per layer (**I37**), `MERGE INTO` for upserts and SCD (**A39**, **E46**), and incremental processing through Structured Streaming or CDF rather than full rebuilds.

**Why** — the pattern earns its place for one reason: **replayability**. Keeping Bronze immutable and raw means every downstream error is recoverable by reprocessing rather than by asking the source system for history it may not have. Everything else about it is a convention you should be willing to argue with, and current practice does argue with it. The failure mode named repeatedly in the field is a **Silver layer that is only "Bronze but slightly cleaner"** — a few nulls dropped, some duplicates removed — which produces three copies of the same data, three times the storage, and no clearer contract than one. Three layers is a default, not a law: a small pipeline with one trustworthy source often wants two, and a heavily-modelled warehouse often wants more than three. The questions that decide it are ownership (who may write each layer), the contract (what a consumer may assume), and retention (how far back each layer goes) — decide those and the number of layers falls out; skip them and the layers collapse into each other within a year.
...
**Estimated time:** 97–134 hrs · **63 topics**
...
**Milestone** — get the full four-plan output with `df.explain(True)` — or equivalently `df.explain("extended")` or `df.explain(mode="extended")`, since a string in the first slot is read as the mode. Note first what you *cannot* write: `df.explain(True, "extended")` raises `PySparkValueError` with condition `CANNOT_SET_TOGETHER`, because `extended` and `mode` are mutually exclusive rather than complementary, and there is no two-argument `EXPLAIN` in SQL either. Then identify which stage shuffles, and verify a filter was pushed below a join. From the analyze phase: name the rule that turns an `UnresolvedAttribute` into a bound column; explain why a self-join needs `DeduplicateRelations`; distinguish an `AnalysisException` from a runtime error. From the optimize phase: set `spark.sql.planChangeLog.level=INFO` with `planChangeLog.rules` pinned to one rule and read the before/after diff on your own query, then exclude that rule with `spark.sql.optimizer.excludedRules` and show the difference.
...
**What** — two decisions `UnionExec` makes that used to be "no" unconditionally. **Output partitioning** (`spark.sql.unionOutputPartitioning`, true since 4.1): if every child reports the same partitioning after rewriting each one's attributes into the first child's, the union reports it too, so an aggregate above needs no re-shuffle. **Codegen fusion** (`spark.sql.codegen.wholeStage.union.enabled`, 4.2.0, default **`false`**): the children fuse into one generated loop over a `UnionRDD` unless one of eight disqualifiers fires — `union-codegen-disabled`, `partitioning-aware`, `nested-union`, `multi-rdd-child`, `partition-index-dependent-child`, `max-children-exceeded`, `columnar`, `type-mismatch`, in that order, memoized on the operator as `supportCodegenFailureReason`.

**Why** — a union sits in the middle of most real pipelines (backfill plus increment; several sources normalised to one schema) and until 4.x cost both an extra shuffle and a codegen break every time. **The two halves are not in the same state, and the difference is the whole practical point.** The partitioning half is on: `spark.sql.unionOutputPartitioning` is `.version("4.1.0")` and defaults to `true`, so the extra shuffle really is gone by default. The codegen half is **not**: its config arrived in 4.2.0 defaulting to `false`, which means out of the box the *first* disqualifier fires and every `UnionExec` still breaks the generated stage exactly as it did in 3.x. Read a release note and you would think both landed; read `SQLConf` and only one is switched on.

On seeing which case you are in: the *reason* is `logDebug` only, so there is no `EXPLAIN` line and no metric carrying the string. But there is a signal the topic used to deny — `UnionExec.metrics` registers `numOutputRows` **only when fusion will actually run**, and returns `Map.empty` otherwise, deliberately so a fallback does not show a stuck 0-valued count. So the presence or absence of a row count on the Union node in the SQL tab tells you *whether* it fused; only the DEBUG line tells you *why not*. The partitioning half also has a deliberately strict equality rule: two `RangePartitioning`s with the same ordering and count are **not** equal, because their bounds were sampled independently.
...
**Milestone** — union two DataFrames already hash-partitioned on the same key, aggregate above, and confirm no exchange sits between union and aggregate — then set `spark.sql.unionOutputPartitioning=false` and show it reappear. Separately, run a six-branch union with `DEBUG` on for `UnionExec` and record which disqualifier fired — on a default cluster the answer is `union-codegen-disabled`, and getting that answer *first* is the point: set `spark.sql.codegen.wholeStage.union.enabled=true` and run it again to reach a real one. Then check the Union node in the SQL tab in both runs and say what the presence or absence of `numOutputRows` told you that `EXPLAIN` did not.
...
**What** — every AQE re-plan is compared against the current plan by a `CostEvaluator` before adoption. The default is `SimpleCostEvaluator`, and it has **two** modes, chosen by the flag `AdaptiveSparkPlanExec` passes into its constructor from `spark.sql.adaptive.forceOptimizeSkewedJoin` (3.3.0, default **`false`**). At the default the entire cost function is the number of `ShuffleExchangeLike` nodes in the plan — so a re-plan that removes a shuffle wins and one that trades a shuffle for a far cheaper join loses. Turn the flag on and the cost becomes `-numSkewJoins << 32 | numShuffles`, which makes the count of skew-handled joins dominate and demotes shuffles to a tiebreaker, so a plan with *more* shuffles can now win if it splits more skew. That flag, not just `spark.sql.adaptive.customCostEvaluatorClass`, is a supported way to change the rule — and it is aimed squarely at the case this topic is about.

The adoption test itself is the part to read literally (`AdaptiveSparkPlanExec`): the new plan is taken when `newCost < origCost` **or** when `newCost == origCost && currentPhysicalPlan != newPhysicalPlan`. Equal cost plus any structural difference is enough.

**Why** — because the adoption gate **ignores data size entirely**: it counts plan nodes. A re-plan that would read a tenth as much data loses to the current plan if it adds one shuffle, and nothing reports that it happened. Knowing the gate exists is what stops an afternoon of threshold tuning against a decision that never looked at your thresholds — and the fix for the common case is the `forceOptimizeSkewedJoin` flag, not a custom evaluator class.

The diagnostic trap worth having straight, because it is the opposite of what you would guess: **a discarded re-plan leaves no trace in the log at all.** `logOnLevel("Plan changed:")` sits *inside* the adoption branch, after the cost comparison has already passed — so seeing `Plan changed:` means the new plan **was** kept. There is no "considered and rejected" line to grep for, which is exactly why the discard is hard to notice and why this topic exists. The only way to see a rejection is to make the two costs visible yourself, by installing a `CostEvaluator` that logs what it is asked to score.

**Learn** — no book covers this · docs: [Adaptive Query Execution](https://spark.apache.org/docs/latest/sql-performance-tuning.html#adaptive-query-execution) · source: `sql/core/…/execution/adaptive/simpleCosting.scala` is the whole cost function in about twenty lines — read both branches of `SimpleCostEvaluator.evaluateCost` and the `SimpleCost.compare` beneath it; `sql/core/…/execution/adaptive/AdaptiveSparkPlanExec.scala` is where the evaluator is built from `ADAPTIVE_FORCE_OPTIMIZE_SKEWED_JOIN`, and where the two-clause adoption test and the `Plan changed:` log line sit — note which side of the `if` the logging is on · sweep [adaptive](reference/spark-source-map/sweeps/sql-core-adaptive.md)

**Milestone** — set `spark.sql.adaptive.logLevel=INFO`, run a query where AQE converts a sort-merge join to a broadcast join, find the `Plan changed:` output and state which plan had fewer shuffles. Then answer, before testing it, what a `CostEvaluator` returning a **constant** would do — the intuitive answer is "no plan is ever adopted", and it is wrong: equal cost plus a structurally different plan satisfies the second clause of the adoption test, so re-plans are adopted anyway. Verify that, then get what the constant evaluator was supposed to give you by writing one that *logs* both plans it is asked to score — which is the only way to observe a re-plan that was computed and rejected, since rejection prints nothing. Finally, flip `spark.sql.adaptive.forceOptimizeSkewedJoin` on a skewed join and say, from the cost formula rather than from timings, why a plan with an extra shuffle can now win.
...
**Why** — it explains two things that otherwise look like bugs: a plan visibly smaller in the SQL tab than in `df.explain()`, and stages that appear in the plan but never run. Sharp edges, all three readable in `AQEPropagateEmptyRelation` itself. The row-count contract is **asymmetric**, and the comment above `getEstimatedRowCount` states it in three lines: **zero means the plan *must* produce zero rows**, a positive value is an estimate that *may be over-estimated*, and `None` means not yet materialized or not estimable. So emptiness is never a guess — the rule acts on zero because zero is exact by construction, not because it checked an exactness flag. A user's root-level `repartition` is deliberately exempted (`isRootRepartition` tests a `ROOT_REPARTITION` tag), so the rule will not delete a repartition the user asked for at the top of the plan. And a broadcast query stage is never eliminated on its own — `canExecuteWithoutJoin` returns `false` for one, because the broadcast is the thing another operator is waiting on.
...
> **The bloom filter itself changed in 4.1.** `spark.sql.optimizer.runtime.bloomFilter.*` governs *whether* a runtime filter is planted; SPARK-47547 replaced the implementation underneath with **BloomFilter V2** and made it the default. It matters for exactly one reason: false-positive behaviour and serialized size changed, so a 4.1 upgrade can shift how much a runtime filter actually skips without any config of yours moving — and the same V2 implementation is what `bloom_filter_agg` and the write-side filters in **A48** use.
...
> **New in 4.2.0 — a join primitive that is not one of the four.** The `NEAREST BY` SPIP (SPARK-56395) adds a top-K ranking join: `… (APPROX | EXACT) NEAREST <k> BY (DISTANCE | SIMILARITY) <expr>` is its own grammar clause, not a hint on an equi-join. It answers "for each row on the left, the k closest rows on the right" — nearest-neighbour retrieval, entity matching, embedding lookup (**A23**) — which none of the four strategies in this topic expresses, because there is no equality to hash or sort on. The `APPROX`/`EXACT` choice is the interesting axis: exact is a ranking over a full comparison, approximate trades recall for a plan that scales. Reach for it before building k-nearest search out of a cross join and a window function, which is what people did until 4.2.

#### ⬜ A16 — Join-Side Buffering and Spill: Why One Key Kills a Task

`v1: A30`

**What** — every join operator except broadcast hash join holds rows inside the task. A sort-merge join buffers **all buffered-side rows sharing the current key**; a shuffled hash join builds an entire partition's `HashedRelation` in task memory; a cartesian product buffers a whole right partition; a broadcast nested loop join holds the full broadcast array plus a `BitSet`. Each has its own thresholds in its own config namespace — and the hash relations cannot spill at all — a fact the source states about as bluntly as it can, since `HashedRelation` implements the `MemoryConsumer` contract with `def spill(size: Long, trigger: MemoryConsumer): Long = 0L`. The method exists because the interface demands one; it releases nothing, so memory pressure has no lever to pull here.
...
**What** — the vectorised Python UDF family, which is one decision tree rather than a list. `pandas_udf` comes in **SCALAR** (a `Series` in, a `Series` out, one batch at a time), **SCALAR_ITER** (an iterator of batches, so expensive setup happens once per partition rather than per batch) and **GROUPED_AGG** (a whole group in, one value out) — and each has an **Arrow-native twin** (`SQL_SCALAR_ARROW_UDF`, `SQL_SCALAR_ARROW_ITER_UDF`, `SQL_GROUPED_AGG_ARROW_UDF`) that skips the pandas conversion. A `GROUPED_AGG` udf is also what you attach to a **window**, which is the case with the sharpest cost: the frame is materialised per row-group before your function sees it.

**Why** — the choice between them is about *where the cost is*, and only one of the three is obvious from the signature. A model loaded inside a SCALAR udf is loaded per batch; the same code as SCALAR_ITER loads it once per partition, which on a 200-partition job is the difference between 200 loads and thousands. A GROUPED_AGG over a window looks like a cheap aggregate and is not: an unbounded or wide frame means the group is assembled in memory per row, which is the same buffering problem **A16** describes for joins. And since 4.2.0 the ground has moved underneath all of this — Arrow-optimised plain Python UDFs are on by default (**I10**), so the old rule "always prefer a pandas UDF to a Python UDF" is no longer automatically true and should be re-measured rather than assumed.
...
**Why** — every exactly-once claim, every "my query reprocessed a batch after restart", and every checkpoint-corruption incident resolves to the ordering of those two writes and what recovery reads back — and none of it is visible from the DataFrame API. Checkpoints are therefore the part of streaming operations people learn by outage.
...
**What** — the stateful half of Structured Streaming: aggregations that must remember, and the machinery that keeps that memory bounded. **Watermarks** (`withWatermark(eventTime, delay)`) declare how late an event may be, which is what lets Spark drop state for closed windows and decide what to emit in append mode. **Windows** — tumbling, sliding and session — group events by event time rather than arrival. **Stream–stream joins** hold both sides in state and need watermarks on both to be bounded. **Deduplication** comes in two forms: `dropDuplicates` keeps every key forever, `dropDuplicatesWithinWatermark` bounds it. For arbitrary logic there are two generations: `flatMapGroupsWithState` / `applyInPandasWithState`, and the newer `transformWithState` (**A59**), which is where new work goes.

**Why** — because every one of those is really the same question — *how much state, for how long* — and until 4.0 the watermark was the only answer Spark had — **A59**'s per-variable TTL is now a second one, bounded per state variable rather than per row. Without either, state grows without bound and the job dies days later with an OOM that names none of this; with one set too tight, late events are dropped silently and your counts are quietly wrong. The asymmetry worth internalising: **a watermark is a promise about lateness, not a guarantee about completeness**. Output mode compounds it — append emits a window only once the watermark has passed it, so a tight watermark trades correctness for latency and a loose one trades latency for state. The 4.x additions do not change that trade; they give you finer control over it (**A59**'s per-variable TTL) and the ability to look at the state you are holding (**A60**).
...
**What** — the operations that make a Delta table maintainable, and the table properties that arm them. **`MERGE INTO`** for upserts and SCD Type 1/2, **`OPTIMIZE`** to compact small files with **`ZORDER BY`** for multi-column locality, and **liquid clustering** (`clusterBy`) as the newer alternative that avoids re-partitioning the table by hand. **Deletion vectors** (`delta.enableDeletionVectors`, default **`false`** at 4.3.1) switch deletes and updates from rewriting whole files to writing a bitmap of removed row positions beside them — merge-on-read rather than copy-on-write, which is the same choice **E54** describes generically for DSv2. **Change Data Feed** (`delta.enableChangeDataFeed`, default **`false`**) records row-level changes into a `_change_data/` directory readable as a stream or a batch. Alongside: **row tracking** (`delta.enableRowTracking`), schema evolution on `MERGE`, and `VACUUM` as the reclaim step.

**Why** — because each of these is a *write-side* decision that a reader inherits, and two of them are off by default in a way that matters retroactively. Deletion vectors change what a one-row `DELETE` costs — a bitmap rather than a rewritten 512 MB file — but enabling them only affects operations *from then on*, and they raise the table's protocol requirements so an older reader may refuse the table entirely (**E33**). Change Data Feed is worse in that respect: it records changes **only for commits made after it was enabled**, so turning it on during an incident gives you nothing about the incident. `OPTIMIZE` with `ZORDER` is the classic over-application — it rewrites data to co-locate values of the columns you name, which pays only if those columns are the ones you filter on, and costs a full rewrite each time; liquid clustering exists because that choice is hard to get right once and impossible to change cheaply. And `MERGE` deserves its reputation: it reads the target to find matches, so a merge with a non-selective condition is a full outer join in disguise — the file-skipping you get from clustering is what makes it viable.

**Learn** — DLDG Ch 4–7; DLUR Ch 5–7 · docs: [Delta table optimizations](https://docs.delta.io/latest/optimizations-oss.html), [deletion vectors](https://docs.delta.io/latest/delta-deletion-vectors.html), [Change Data Feed](https://docs.delta.io/latest/delta-change-data-feed.html), [MERGE](https://docs.delta.io/latest/delta-update.html) · source: `spark/src/main/scala/org/apache/spark/sql/delta/DeltaConfig.scala` in the 4.3.1 checkout is where `enableDeletionVectors`, `enableChangeDataFeed` and `enableRowTracking` state their defaults; the `commands/` directory holds `OptimizeTableCommand`, `MergeIntoCommand` and `VacuumCommand` · related: **E54** (the same copy-on-write versus merge-on-read choice in DSv2 terms), **E46** (CDC patterns, of which CDF is one implementation), **E33** (what enabling these does to the protocol), **I32** and **I52**'s `CLUSTER BY` note, **A18**, **A61** (the same jobs on Iceberg, where none of them run unless you call them)

**Milestone** — delete one row from a large table with deletion vectors off, then on, and compare files written in each case; then say what a reader that does not support deletion vectors does with the second table. Enable Change Data Feed, make three changes, read the feed — then answer the question that matters: what does the feed say about the changes you made *before* enabling it? Run `OPTIMIZE … ZORDER BY` on a column you actually filter on and measure files scanned before and after; repeat on a column you never filter on and say what you paid. Finally, write a `MERGE` whose condition matches nothing and explain, from the plan, why it was still expensive.

#### ⬜ A40 — Spark Declarative Pipelines

`v1: A11` · new in Spark 4.1, runs over Spark Connect

**What** — Spark Declarative Pipelines: you declare *datasets* and the flows that produce them, and the engine resolves the dependency graph, orders execution and manages the update. In PySpark the surface is decorators from `pyspark.pipelines` — `@materialized_view` and `@table` (both taking `name`, `comment`, `spark_conf`, `table_properties`, `partition_cols`, `cluster_by`, `schema`), `@temporary_view`, and `@append_flow` for adding a flow into an existing target. A pipeline is described by a spec file and run through the **`spark-pipelines`** CLI (`spark-pipelines run --spec pipeline.yaml`), which also carries `--full-refresh` and `--full-refresh-all`. Dependencies are inferred from the queries themselves, not declared by hand.

**Why** — because the alternative is the thing every team builds twice: a DAG of jobs whose edges live in an orchestrator while the actual dependencies live in SQL, and drift between the two is the outage. Declaring the dataset instead of the task means the graph is derived from the code that reads the tables, which is the same argument **E47** makes for Dagster's asset model — the difference being that this one runs *inside* Spark, so the graph resolves against the catalog and the engine owns retries and ordering (**E43**) rather than an external scheduler. The trade to be clear about: you gain a dependency graph you cannot desynchronise, and you give up imperative control over execution — which is exactly why `--full-refresh` exists as a CLI flag rather than as something your code decides.
...
**Learn** — SDG Ch 16; DEB Module 4 · docs: [Testing PySpark](https://spark.apache.org/docs/latest/api/python/getting_started/testing_pyspark.html) — including `assertDataFrameEqual`, which is built in since 4.0 and replaces most of what third-party libraries were for · source: `python/pyspark/testing/utils.py` — the `assertDataFrameEqual` signature is the whole contract, and every default in it is a decision about what "equal" means · do this before **E48** (CI/CD), which automates it
...
> **The optional capability nobody expects: indexes.** `SupportsIndex` (SPARK-36526, 3.3) puts `CREATE INDEX` / `DROP INDEX` / `SHOW INDEXES` into Spark SQL for connectors that have indexes to create. Exactly one built-in implements it — `JDBCTable` — so the statement is a passthrough to your database's DDL through the dialect, not anything Spark maintains. Know it mainly so the syntax existing does not mislead you: on any table format without the mix-in it is an analysis error, and on JDBC it is your database's index, with your database's cost.
...
> **The v2 command surface only just caught up.** For years a v2 catalog answered `SELECT` and `INSERT` fully but fell over on inspection and DDL, and the gap closed release by release: `SHOW COLUMNS` (SPARK-49078, 4.0), `ALTER NAMESPACE … UNSET PROPERTIES` (48668, 4.0), `listTableSummaries` (52109, 4.1), then a burst in 4.2.0 — `CREATE TABLE LIKE` (33902), `DESCRIBE TABLE … PARTITION` (39660), the v2 view DDL and inspection commands (56655), and structured catalog/namespace/table rows in `DESCRIBE TABLE EXTENDED` (56678) — with `ViewCatalog` as the interface a connector implements to take part in any of it. The practical rule: "does this command work on my catalog" is a **version** question as much as a connector one, and an answer from a 3.x-era blog post is worth re-testing.
>
> **And one item on that list is the opposite of a feature, which is why this page reads the tree.** An earlier pass recorded SPARK-52729 here as `MetadataOnlyTable` bringing `CREATE`/`ALTER VIEW` to v2 catalogs. At tag `v4.2.0` **no such type exists anywhere in the OSS tree**, and the ticket number appears exactly once — in a comment in `DataSourceV2Strategy.scala` reading "Tracked for follow-up real handlers in SPARK-52729", above `case` arms that make `REFRESH TABLE` and `ANALYZE TABLE` on a non-session v2 view throw `UNSUPPORTED_FEATURE.TABLE_OPERATION`. So the 4.2.0 change was not that those commands started working; it was that they stopped failing as a planner assertion and started failing as a clean `AnalysisException`, with the real implementation still open. Making a gap *legible* and closing it look identical in a release note and opposite in the source — the same shape as **B12**'s `ai_complete`, and the second time on this page that a name from a release note has resolved to nothing.
...
### Strand — Keeping a table maintained

#### ⬜ A61 — Iceberg Table Maintenance: Rewrite, Expire, and the Orphan-File Footgun

**New topic** · no v1 code · sourced from a coverage review of the lakehouse strand rather than from a feature-history audit: **A39** owns Delta's maintenance surface and **I38** teaches Iceberg's *read* model — the manifest tree, hidden partitioning, field ids — but nothing on this page owned Iceberg's *upkeep*, whose procedure names, defaults and destructive edges are entirely different from Delta's and are not derivable from knowing Delta's

**What** — Iceberg exposes maintenance as **stored procedures**, not as SQL commands: `CALL <catalog>.system.<procedure>(...)`, available only when the Spark SQL extensions are registered. Verified against the local checkout at `C:/opt/learn/spark/repos/iceberg`, tag `apache-iceberg-1.11.0`, where the 20 procedures `SparkProcedures` registers do four jobs. **Compaction** — `rewrite_data_files` bin-packs small files, which is what you get when you pass neither `strategy` nor `sort_order`; `strategy => 'sort'` rewrites them clustered by a sort order instead, and a `sort_order` of `zorder(a, b)` gives Z-ordering, the same locality trade **A39** describes for Delta. Beside it, `rewrite_manifests` compacts the *metadata* tree rather than the data, and `rewrite_position_delete_files` compacts the merge-on-read delete files that accumulate under **E54**'s delta-based rewrite. **Expiry** — `expire_snapshots` drops old snapshots and the data files only they referenced, which is what actually bounds time travel. **Cleanup** — `remove_orphan_files` deletes files under the table location that no snapshot references. **Statistics** — `compute_table_stats` and `compute_partition_stats` write **Puffin** files holding Theta-sketch NDV blobs, which reach Spark by a different route than **A10**'s `ANALYZE TABLE` statistics: `SparkScan` implements DSv2 `SupportsReportStatistics`, and its `estimateStatistics` only populates column stats when **both** `spark.sql.cbo.enabled` (Spark's own, default **`false`**) and `spark.sql.iceberg.report-column-stats` (Iceberg's, default **`true`**) are on. Computing the sketches therefore changes no plan until you enable the CBO. Around those sit the snapshot-manipulation procedures (`rollback_to_snapshot`, `set_current_snapshot`, `cherrypick_snapshot`, `publish_changes`, `fast_forward`, `create_changelog_view`) and the onboarding ones (`migrate`, `snapshot`, `add_files`, `register_table`).

**Why** — because **none of it runs on its own**, and the defaults that decide what happens when it does are not the ones a Delta reader expects. Three, read from `core/src/main/java/org/apache/iceberg/TableProperties.java` at 1.11.0. `history.expire.max-snapshot-age-ms` is **5 days** and `history.expire.min-snapshots-to-keep` is **1** — but that pair is only consulted *when you call `expire_snapshots`*, so a table nobody maintains keeps every snapshot forever and its metadata grows without limit. `write.metadata.previous-versions-max` is **100** while `write.metadata.delete-after-commit.enabled` is **`false`**, so the cap is not enforced unless you enable it — this is a *separate* accumulation from snapshots, and the reason a table with a healthy snapshot count can still carry tens of thousands of metadata JSONs. `write.target-file-size-bytes` is **512 MB** and `write.delete.target-file-size-bytes` is **64 MB**. On the rewrite itself (`SizeBasedFileRewritePlanner` and `RewriteDataFiles`): a partition with fewer than `min-input-files` = **5** eligible files is skipped unless `rewrite-all` = `true`, groups cap at `max-file-group-size-bytes` = **100 GB** with `max-concurrent-file-group-rewrites` = **5**, and `partial-progress.enabled` is **`false`** — so by default a multi-hour compaction commits once at the end and a failure at hour three loses all of it, where enabling it commits in up to `partial-progress.max-commits` = **10** batches.

Then the one that destroys data. **`remove_orphan_files` is the only maintenance operation that does not work from metadata alone** — it lists the filesystem under the table location and diffs that against the table's known file set, so anything that makes the two lists spell the same file differently makes live data look orphaned. `DeleteOrphanFilesSparkAction` defends with `prefixMismatchMode` defaulting to **`ERROR`**, and the detail that decides which mismatches are dangerous is that it carries **two** normalisation maps with **different defaults**: `equalSchemes` is pre-populated with `s3n,s3a → s3`, while `equalAuthorities` defaults to an **empty map**. So the case everyone names first — a catalog on `s3://` against a listing on `s3a://` — is exactly the one already handled, and the unguarded case is an **authority** mismatch: the same data reached through two bucket or endpoint names, or an HDFS nameservice alias against the concrete namenode host, which conflicts by default because nothing pre-equates authorities. Schemes outside that one built-in pair — `hdfs` versus `viewfs`, `abfs` versus `abfss` — conflict too. The way people delete their own table is setting the mode to `DELETE` to make the resulting error stop, rather than adding the pair to `equalSchemes()` or `equalAuthorities()` as the message actually asks. Two more guards worth knowing before you need them: the *procedure* refuses an `older_than` more recent than **24 hours** — the message names concurrent operations as the reason — while the *Action API* has no such check, and the default is **now − 3 days** either way; and `gc.enabled` (default **`true`**) is a table property that disables the destructive actions entirely, which is what you set on a table whose files another engine's snapshots still reference.

Two things this topic is also the place to settle. First, **who runs it.** Every procedure here is a job someone must schedule, size and monitor, and that is the gap a whole product category sells into — Apache Amoro (incubating) as the open-source self-hosted answer, which is **A63**, and managed layers such as Databricks predictive optimization, Snowflake managed Iceberg, AWS S3 Tables and LakeOps above your existing catalog. Reaching for one is a reasonable decision; reaching for one *before* you can say which procedure you needed and why is how a compaction bill outgrows the query bill it was meant to reduce. On this path the scheduler is **E47** and the thing that tells you it worked is **E24**. Second, **the version lag.** Iceberg 1.11.0 ships Spark modules for 3.4, 3.5, 4.0 and 4.1 and *no 4.2 module* — so a cluster on the 4.2.0 this page targets has no runtime jar to load, and "which Spark can I actually run" is a question you ask the connector, not the engine.

**Learn** — Iceberg-DG's "Iceberg Table Maintenance Procedures" section, which walks `expire_snapshots`, `rewrite_data_files`, `rewrite_manifests` and `remove_orphan_files` in that order · docs: [Maintenance](https://iceberg.apache.org/docs/latest/maintenance/) — note that it splits its own advice into *recommended* (expire snapshots, remove old metadata files, delete orphan files) and *optional* (compact data files, rewrite manifests), which is the priority order to adopt them in; [Spark Procedures](https://iceberg.apache.org/docs/latest/spark-procedures/) for the full call signatures; [Configuration](https://iceberg.apache.org/docs/latest/configuration/) for the write and history properties · source: the local Iceberg checkout at tag `apache-iceberg-1.11.0` — `core/…/TableProperties.java` carries every default quoted above, `core/…/actions/SizeBasedFileRewritePlanner.java` has `min-input-files` and the file-group caps, `api/…/actions/RewriteDataFiles.java` has the partial-progress options, and `spark/v4.1/…/procedures/SparkProcedures.java` is the registry that settles how many procedures there actually are — count that, not the files in the directory, three of which are not procedures. `RemoveOrphanFilesProcedure.validateInterval` holds the 24-hour refusal; `DeleteOrphanFilesSparkAction` holds the `ERROR` default, the `s3n,s3a → s3` scheme map **and the empty authority map beside it**, which is the pair to read together; `spark/v4.1/…/source/SparkScan.java` shows the Puffin blobs being read behind a `spark.sql.cbo.enabled` check, with the Iceberg-side default in `SparkSQLProperties.REPORT_COLUMN_STATS_DEFAULT` · prerequisite: **I38** · related: **A39** (the same jobs on Delta, under different names and different defaults), **E33** (why the two formats reason about concurrency differently), **E54** (the delete files `rewrite_position_delete_files` cleans up), **A10** (the statistics the compute procedures write), **A62** (Paimon, where the same jobs run automatically inside the commit and the danger is the opposite one), **E47** (scheduling this), **E24** (knowing it ran), **I36**, **A47**

**Milestone** — take an Iceberg table and write to it enough times to make it unhealthy: many small files and a long snapshot history. Before running anything, predict from `min-input-files` alone which partitions `rewrite_data_files` will touch, then run it and check whether you were right. Run it a second time with no changes in between and explain why it did much less work. Then `expire_snapshots` and demonstrate the consequence that matters — a time-travel read that worked before and fails after — and name which of the two history properties decided it. Separately, count the files in the metadata directory, enable `write.metadata.delete-after-commit.enabled`, commit again, and say why the count changed when no snapshot was expired. Finally, the safety one, on a table you are willing to lose. First predict, then test: try to provoke the prefix-mismatch error using `s3://` against `s3a://` and say *before running it* whether it will fire — the answer is no, and naming the map that absorbs it is the point of the exercise. Then provoke it for real with a mismatch nothing pre-equates: reach the same data through two authorities, or through a scheme pair outside the built-in one. With the error in hand, explain from how the procedure builds its two lists exactly what `prefix_mismatch_mode => 'DELETE'` would have deleted, and say which of `equalSchemes()` or `equalAuthorities()` was the correct fix.

### Strand — The streaming-first table format

#### ⬜ A62 — Apache Paimon: LSM Tables, Merge Engines, and Streaming Upserts

**New topic** · no v1 code · sourced from the same lakehouse-coverage review that added **A61**, extended after Apache Amoro turned out to manage Paimon as a first-class format beside Iceberg: this page taught the two formats that share a design — immutable data files plus a metadata log (**I37**, **I38**) — and had no topic for the third, which does not share it

**What** — verified against the local checkout at `C:/opt/learn/spark/repos/paimon`, tag `release-2.0.0`. Paimon splits tables in two. An **append table** behaves much like Iceberg. A **primary-key table** is an **LSM tree**, and that is the whole difference: writes accumulate in a memory buffer (`write-buffer-size`, **256 MB**), spill as *sorted runs*, and background compaction merges runs down through levels — `num-sorted-run.compaction-trigger` = **5** is when that starts. Rows are hash-distributed into **buckets** (`bucket`, default **`-1`**, meaning dynamic-bucket mode), and a bucket is the unit of both an LSM tree and write parallelism. Because a read merges sorted runs on the fly, the format needs a rule for what happens when two rows share a key, which is the **merge engine** (`merge-engine`, default **`deduplicate`**): `deduplicate` keeps the last row, `first-row` keeps the first, `partial-update` fills non-null fields from each arriving row, `aggregation` applies per-column aggregate functions. `sequence.field` decides what "last" means and has **no default**; with none set, Paimon assigns the sequence number itself — `MergeTreeWriter` starts at the bucket's existing `maxSequenceNumber + 1` and increments per record as it writes — so "last" means *last written by that writer into that bucket*, which is not the same as last to arrive at the job. Separately, the **changelog producer** (`changelog-producer`, default **`none`**) decides whether the table can emit a real update-before/update-after stream to downstream consumers, with `input`, `full-compaction` and `lookup` as the three ways to get one, each trading write cost against changelog latency. Spark reaches all of it through a DSv2 catalog plus a `CALL` procedure set of Paimon's own — `compact`, `expire_snapshots`, `expire_partitions`, `rollback_to`, `create_tag`, `migrate_table`, `remove_orphan_files` and some forty more — and Spark is a full participant rather than a reader: `PaimonMicroBatchStream` provides streaming reads and `SparkSource` implements `StreamSinkProvider` for streaming writes.

**Why** — because retention here runs **the opposite way from A61**, and that single contrast teaches what the format is for. On Iceberg nothing expires until you call the procedure. On Paimon expiry is *part of the commit*: `TableCommitImpl` runs it in the post-commit block, `snapshot.expire.execution-mode` defaults to **`SYNC`**, and it drops up to `snapshot.expire.limit` = **50** snapshots per commit. The budget it enforces is small — `snapshot.num-retained.min` is **10** and `snapshot.time-retained` is **one hour**. A Paimon table under steady writes therefore keeps roughly its last ten snapshots or its last hour, where Delta keeps 30 days of log (**I37**) and an unmaintained Iceberg table keeps everything forever. **Time travel on Paimon is a streaming-restart window, not a history**, and either of the other two formats would call that data loss. It is the design rather than a defect: Paimon is built for a stream of upserts in which the current state is the product and the history is exhaust — which is also why keeping something long-term is a **tag**, a named snapshot retained on its own schedule, instead of an expectation that the snapshot list will still hold it.

Two more things decide whether you get this right. **The merge engine is a write-time contract you cannot cheaply re-decide**, because it is applied during compaction — rows already merged under one engine cannot be un-merged under another. `partial-update` carries the sharp edge: it fills *non-null* fields, so a genuine `NULL` can never overwrite a value, and "that column stopped updating" is the shape the bug takes. And **the version lag is the same lesson A61 ends on**: Paimon 2.0.0 ships Spark modules for 3.2 through 4.1 and **none for 4.2**, Scala 2.13 only across the 4.x line. The connector decides which Spark you may run, not the engine.

**Learn** — no book on this path covers Paimon: the Iceberg and Delta books predate it as a competitor and none of the Spark books mention it, so the docs and the source are the whole of it · docs: [Primary Key Table](https://paimon.apache.org/docs/master/primary-key-table/overview/) — the LSM, buckets and sorted runs, and the page to read first because everything else follows from it; [Merge Engine](https://paimon.apache.org/docs/master/primary-key-table/merge-engine/overview/) for the four engines side by side; [Spark quick start](https://paimon.apache.org/docs/master/spark/quick-start/) for catalog wiring and the version matrix · source: the local checkout at tag `release-2.0.0` — `paimon-api/src/main/java/org/apache/paimon/CoreOptions.java` holds every default quoted here (`merge-engine`, `changelog-producer`, `bucket`, the three `snapshot.*retained` options, `snapshot.expire.execution-mode` and `.limit`, `write-buffer-size`, `num-sorted-run.compaction-trigger`), `paimon-core/…/table/sink/TableCommitImpl.java` is where expiry runs *inside* the commit, `paimon-core/…/mergetree/MergeTreeWriter.java` is where a record gets its sequence number when `sequence.field` is unset, `paimon-spark/` shows which Spark versions actually ship, and `paimon-spark/paimon-spark-common/…/spark/procedure/` is the full procedure list · prerequisites: **I38**, **A34** · related: **A61** (the same maintenance jobs on Iceberg, where none of them are automatic), **I37**, **E33** (a third answer to the same concurrency question), **E46** (CDC, which is what a changelog producer feeds), **E54** (row-level DML solved by rewriting rather than by merging), **A32**

**Milestone** — create a primary-key table from Spark and write the same key twice; show that a read returns one row, and name the merge engine that decided which. Recreate it with `partial-update` and demonstrate the trap directly: write a row that sets a column to `NULL` and show the old value surviving. Then find the LSM: write enough small batches to cross `num-sorted-run.compaction-trigger`, and say from the files what compaction did. Commit fifteen times over more than an hour and count the snapshots that remain — explain the number using both `snapshot.num-retained.min` and `snapshot.time-retained`, then keep one of them past its expiry with a tag and say why that is the mechanism rather than raising the retention. Finally, set a `changelog-producer` and read the table as a stream from Spark, saying what a downstream consumer gets that it did not get with `none` — and what the write now costs.

### Strand — Automating the lakehouse

#### ⬜ A63 — Apache Amoro: Self-Optimizing as a Service

**New topic** · no v1 code · sourced from the lakehouse-coverage review that produced **A61** and **A62**, which between them leave one question open: every maintenance operation those topics teach is a job somebody must schedule, size and watch, and this page's only answers were **E47** (write the cron yourself) or a vendor. Amoro is the most complete open-source answer — a management *system* rather than a job you schedule — though not the only one: Floe, OLake Fusion and IOMETE's compaction job all address narrower slices of the same problem, and **A61** names the managed layers. What Amoro automates is worth knowing whether or not you deploy it

**What** — verified against the local checkout at `C:/opt/learn/spark/repos/amoro`, tag `v0.8.1-incubating`. Amoro is a **management system**: not a format, not an engine, and that is the thing to fix in your head first. **AMS**, the Amoro Management Service, is the server — it holds a unified catalog across engines, continuously reads table metadata, and decides what maintenance is due. `CatalogBuilder` accepts five catalog types (`hadoop`, `hive`, `glue`, `custom`, `ams`), each mapped to the formats it may hold, so an existing **Hive Metastore** is something AMS fronts rather than replaces. The work is carried out by **optimizers**: long-lived processes AMS dispatches tasks to, shipped in **three** runtimes — `amoro-optimizer-flink`, `-spark` and `-standalone` — plus an `external` *container type* (`Constants.EXTERNAL_RESOURCE_CONTAINER`), which is not a fourth runtime but the option of launching and registering the process yourself. Optimizers are pooled into **groups**, and each table names the group it draws on (`self-optimizing.group`, default `default`). Optimizing comes in three types — `MINOR`, `MAJOR` and `FULL` — differing in how much they rewrite. Format coverage is wider than any single format's own tooling: `amoro-format-iceberg`, `amoro-format-paimon`, `amoro-format-hudi`, and `amoro-format-mixed` for Amoro's own Mixed-Iceberg and Mixed-Hive.

**Why** — because the model inverts everything **A61** teaches, and because the version matrix decides whether you can use it at all.

The inversion, from `amoro-format-iceberg/…/table/TableProperties.java` at 0.8.1: `self-optimizing.enabled` defaults to **`true`**. Register a table with AMS and it is being rewritten — where an Iceberg table left alone is rewritten never. And you do not schedule the work; you give the table a **share**. `self-optimizing.quota` defaults to **0.1**, a fraction of one optimizer's capacity, and AMS decides when to spend it. That is the conceptual move worth taking away even if you never install this: maintenance as a *budget* against a pool, rather than as a job in an orchestrator. The sizing knobs sit beside it — `self-optimizing.target-size` **128 MB** with `fragment-ratio` **8**, so a file under 16 MB counts as a fragment, and `min-target-size-ratio` **0.75** — and the triggers are thresholds rather than crons: `minor.trigger.file-count` **12**, `minor.trigger.interval` **1 hour**, `major.trigger.duplicate-ratio` **0.1**. Then the one to notice, because it is the same shape as **A61**'s `partial-progress.enabled` and **A62**'s one-hour retention: `full.trigger.interval` is **`-1`**, meaning **full optimizing never runs unless you set it**. The expensive, thorough operation is the one that is off by default, in all three systems, for the same reason — nobody wants to explain an unbidden full rewrite.

The lag is the part to check before believing any of it applies to you. Amoro 0.8.1-incubating builds against **Iceberg 1.6.1** and **Paimon 0.9.0** — well behind the 1.11.0 and 2.0.0 that **A61** and **A62** are written against — pins **Spark 3.3.3** for its Spark optimizer runtime, and ships its own Spark connector, `amoro-mixed-spark`, for Spark **3.2, 3.3 and 3.5 only, with nothing on the 4.x line**. That resolves how this touches a Spark engineer: for Iceberg and Paimon tables **Amoro never becomes a Spark dependency** — you keep reading those through their own connectors and Amoro manages them from outside, as a service. The Spark connector exists only for Amoro's Mixed formats, and on the 4.x cluster this path targets it does not exist for you. So learn the model, which is what every managed equivalent named in **A61** is also selling, and treat adoption as a version-matrix question you answer before the architecture question. It also remains in the **ASF incubator**, which is a statement about governance maturity rather than code quality — and the accurate thing to say about it in a review, rather than either half of the argument people usually have.

**Learn** — no book covers Amoro, and it postdates every book on this page · docs: [Self-Optimizing](https://amoro.apache.org/docs/latest/self-optimizing/) — the three optimizing types and the fragment-versus-segment model, which is the whole idea; the [Amoro documentation](https://amoro.apache.org/) for AMS, catalogs and optimizer groups; [Apache Amoro](https://iceberg.apache.org/docs/latest/amoro/) in the Iceberg docs for how the Iceberg project itself frames it · source: the local checkout at tag `v0.8.1-incubating` — `amoro-format-iceberg/src/main/java/org/apache/amoro/table/TableProperties.java` carries every `self-optimizing.*` default quoted above, `amoro-ams/…/server/catalog/CatalogBuilder.java` lists the five catalog types and which formats each may hold, `amoro-optimizer/` holds the three shipped runtimes, `amoro-format-mixed/amoro-mixed-spark/pom.xml` is where the Spark 3.2/3.3/3.5-only module list is visible, and the root `pom.xml` pins Iceberg 1.6.1, Paimon 0.9.0 and Flink 1.20.0 · prerequisites: **A61**, **A62** · related: **E47** (the alternative, where you own the schedule), **E24** (how you would know either one worked), **E29** (AMS is also a catalog, which is where this overlaps governance), **I37**, **I38**

**Milestone** — before installing anything, answer the version question in writing: given the Iceberg or Paimon version you actually run and the Spark you actually run, say whether Amoro 0.8.1-incubating can manage your tables, and which of the two answers — "yes, as an external service" or "no" — applies to which format. Then stand up AMS with a standalone optimizer against a local Iceberg table, register it, and watch a minor optimize fire: report which trigger caused it, `file-count` or `interval`. Set `self-optimizing.quota` deliberately low and describe what changes — not "it gets slower", but what AMS does differently with the pool. Find a table whose files are mostly fragments and say, from `target-size` and `fragment-ratio` alone, what the threshold was. Finally, the one that matters: explain why `full.trigger.interval` is `-1`, what you would have to be sure of before setting it, and how the same decision appears under a different name in **A61** and **A62**.
...
**Why** — two symptoms have no other explanation. A cached iterator you never fully drain keeps its read lock and pins the block against eviction for the rest of the task. And the log line `N block locks were not released by task X` is unreadable without the protocol, yet it is reported at INFO by default, so a genuine leak is invisible in most production log configurations. "By default" is doing real work in that sentence: the same branch in `Executor.scala` throws instead of logging when `spark.storage.exceptionOnPinLeak` (default **`false`**) is on — so the choice is not between noticing and not noticing, it is between a line you filtered out and a failed task. Turn it on in test, never in production. The memory-leak check immediately above it has the identical shape, with `spark.unsafe.exceptionOnMemoryLeak` deciding whether "Managed memory leak detected" is a `logWarning` or a thrown internal error.
...
**What** — the catalog layer: the three-level namespace `catalog.schema.table`, what a catalog owns versus what the table format owns, and the competing implementations — **Unity Catalog** (open-source as well as the Databricks service), the **Iceberg REST Catalog** specification, and the Hive Metastore as the legacy baseline. On top of naming sits governance — grants at each level, column-level access control, row filters and column masks, audit logs, table- and column-level lineage, cross-organisation sharing — and it is worth being precise about *where* that lives: the **specification and the open-source implementations give you the object model and grants**, while row filters, column masks and automated lineage are features of the **managed** platforms built on top. Unity Catalog OSS at the 0.5.x line your stack runs is the object model, not the policy engine. The connection point to the engine is **A51**: a catalog is a `CatalogPlugin` registered under `spark.sql.catalog.<name>`, so the governance layer plugs in where naming does. Your local stack demonstrates the shape — Unity Catalog OSS **0.5.1** behind `spark.sql.catalog.unity = io.unitycatalog.spark.UCSingleCatalog` with a `uri` and a token, tables addressed as `unity.default.<table>`.

**Why** — because the catalog is where multi-engine interoperability is actually decided, and it is the one layer you cannot swap later without touching every reference. The split worth internalising: the **table format** (Delta, Iceberg) owns what a table *is* — files, schema, snapshots, transactions; the **catalog** owns what it is *called*, who may read it, and how another engine finds it. That is why the same Delta table can be governed by Unity Catalog or by a Hive Metastore with completely different access semantics, and why "we use Delta" says nothing about your governance posture. The practical asymmetry: grants and row filters are enforced by whatever *reads* the table, so a governance model is only as strong as the weakest engine you allow at the data — a point that matters the moment someone reads the underlying path directly instead of going through the catalog. Learn the shape — namespace, grants, lineage, sharing — rather than one vendor's console, because that is what transfers.
...
**Milestone** — do the runnable half on your own stack first: register a catalog under `spark.sql.catalog.<name>` (**A51**), create a schema and an external Delta table in it, and address it by its three-level name — then say which part of that the catalog owned and which part the table format owned. Now prove the boundary that matters: read the same table's underlying path directly with `spark.read.format("delta").load(...)`, bypassing the catalog entirely, and state what that means for any access control the catalog claims to enforce. Then the part your stack cannot do: write down, from the vendor documentation, what a **managed** catalog adds — row filters, column masks, automated lineage, cross-organisation sharing — and for each one name the component that would have to enforce it and what happens when a query arrives from an engine that does not implement it.
...
**What** — the transaction log as a protocol rather than a directory listing. Every commit is a set of **actions**: `metaData` (schema, partitioning, table properties), `protocol` (what a reader and writer must support), `add` and `remove` (data files, with stats and optional deletion-vector descriptors), `txn` (idempotent streaming writes), `domainMetadata`, `cdc`, and `commitInfo`. Table state is **action reconciliation** — replay the actions in order, later ones winning, so a checkpoint (`.checkpoint.parquet` plus `_last_checkpoint`) is just that replay materialised. Since Delta 3.x the numeric protocol versions have given way to **table features**: a reader or writer declares support for named features (`deletionVectors`, `columnMapping`, `rowTracking`, …), so a client that understands the ones a table uses may read it even if other features exist. Delta 4.x adds **catalog-managed tables**, where the catalog rather than the file system ratifies a commit. Concurrency is **optimistic**: a writer reads a snapshot, prepares actions, and commits by attempting to create the next version file; conflicts are detected by a `ConflictChecker` and surface as named exceptions — `ConcurrentAppendException`, `ConcurrentDeleteReadException`, `ConcurrentDeleteDeleteException`, `MetadataChangedException`, `ProtocolChangedException`, `ConcurrentTransactionException`. The `isolationLevel` table property accepts exactly one value, **`Serializable`**.

**Why** — because this is where "Delta is ACID" becomes a set of concrete failure modes you can predict. The commit is a single atomic file creation, so *durability* is the storage system's problem: object stores without atomic put-if-absent historically needed a coordinating service, and why 4.x's catalog-managed commits exist. The conflict exceptions are the useful part in practice: they are not one error but six, and each says which concurrent operation invalidated your read set — an append conflicting with your read predicate is a different fix from a metadata change, and retrying blindly makes only the first survivable. Table features matter for the opposite reason to protocol versions: they let a table stay readable by older clients, but they also mean "we enabled deletion vectors" can make a table unreadable for one consumer and fine for another, which is invisible until it happens. And the isolation-level property being validated to a single value is worth knowing before you go looking for a knob that does not exist.

**Learn** — DLDG Ch 8–10; the protocol itself is the primary source, not a book · docs: [Delta Lake protocol (`PROTOCOL.md`)](https://github.com/delta-io/delta/blob/master/PROTOCOL.md) — read "Actions", "Action Reconciliation" and "Table Features"; [concurrency control](https://docs.delta.io/latest/concurrency-control.html) for the conflict matrix · source: the 4.3.1 checkout at `C:/opt/learn/spark/repos/delta` — `PROTOCOL.md` is the specification; `spark/.../delta/ConflictChecker.scala` decides which concurrent commits conflict, `DeltaErrors.scala` defines the six exceptions above, and `isolationLevels.scala` is short enough to read whole · related: **E31** (DSv2 catalog transactions — the engine-side equivalent), **A39** (the features whose protocol requirements this explains), **I37**, **E29** (catalog-managed tables), **A47** (why an atomic commit on object storage is not free)

**Milestone** — open `_delta_log/*.json` from a real table and classify every action in one commit by type; then find the same information in the checkpoint and say what the checkpoint saved a reader. Read the `protocol` action and list the table features your table requires. Then produce a conflict on purpose: run two concurrent writers whose predicates overlap, catch the exception, and name which of the six it is and what read set it invalidated — then say which of the six would have been safe to retry unchanged. Finally, explain in two sentences why an atomic commit needs something more than "write a file" on an object store, and what 4.x's catalog-managed commits change about that.
...
**What** — Dagster as the orchestrator around Spark, built on **assets** rather than tasks: `@asset` declares *a thing that exists* — a table, a file, a model — and its `deps` declare which other assets it is built from, so the graph is a data lineage rather than a task order. A `Definitions` object collects assets, jobs, schedules and resources into a **code location** that the Dagster daemon loads (`dagster code-server start -m <module>`); `define_asset_job` selects a subset of the graph to run and `ScheduleDefinition` attaches a cron to it, while **sensors** trigger on external events instead. **Resources** inject configuration and connections rather than hard-coding them, and **IO managers** decide where an asset's output is persisted — the piece that separates "compute" from "where it lands". `dagstermill` runs a notebook as an asset. Verified against the local stack at `C:/opt/learn/spark/spark-delta-unitycatalog` (Dagster **1.13.x**), whose assets reach Spark over **Spark Connect** — `SparkSession.builder.remote("sc://spark:15002")` — and write Delta tables registered in Unity Catalog.

**Why** — the asset model is the reason to use it here rather than cron plus a shell script. When the unit of scheduling is *the table*, a failure tells you which table is stale rather than which script exited non-zero, backfills are expressible per asset and per partition, and the lineage graph is derived from your code instead of maintained beside it. The Spark-specific consequence is worth stating plainly: **Dagster orchestrates, Spark computes, and the two should not share a process**. Reaching Spark over Connect (**E26**, **E60**) means the Dagster worker holds a thin client rather than a driver, so a Spark job dying does not take the orchestrator with it, and the worker's memory is not sized by your shuffle. The failure mode of the alternative — embedding a `SparkSession` in the asset process — is a Dagster run whose memory profile is a Spark driver's, which is exactly the coupling this arrangement exists to avoid. The second thing to internalise is that an asset run occupies a slot for as long as the Spark job takes; long jobs need the run-queue and concurrency limits configured, or one backfill starves everything else.

**Learn** — no book on this path covers Dagster; **DagEss** (*Dagster Essentials*, free) is the fastest way in and is written around exactly this asset model · docs: [Dagster concepts — assets](https://docs.dagster.io/concepts/assets/software-defined-assets), [schedules and sensors](https://docs.dagster.io/concepts/automation), [resources](https://docs.dagster.io/concepts/resources) and [code locations](https://docs.dagster.io/concepts/code-locations) · your own stack is the best reference: `workspace/dagster/dagster_workspace/` in `C:/opt/learn/spark/spark-delta-unitycatalog` has a three-asset graph with `deps`, a `define_asset_job`, two `ScheduleDefinition`s and a `dagstermill` notebook asset in under sixty lines · related: **E60** (the Connect endpoint the assets talk to), **A40** (Spark Declarative Pipelines — the in-engine alternative, and worth comparing directly), **E48** (getting this deployed), **I39**, **E29**

**Milestone** — take a two-stage pipeline you already have and express it as two assets with a `deps` edge; materialise it and read the lineage graph in the UI. Then break the second asset and say what the graph tells you that an exit code would not. Attach a schedule and a sensor to the same job and state which one you would use for a table that arrives irregularly. Move a connection string out of the asset body into a resource. Finally, run a Spark job through an asset over Connect, kill the Spark side mid-run, and describe what happened to the Dagster run — then explain why embedding the driver in the asset process would have made that worse.

#### ⬜ E48 — CI/CD for Data Engineering

`v1: E7`

**What** — getting a pipeline from a branch into production without a human copying files. Four pieces. **Tests that run in CI**: unit tests over transformation functions, plus DataFrame assertions with `pyspark.testing`'s `assertDataFrameEqual` (**A43**) — and, where the production job runs on Kubernetes or Connect, at least one integration test in an environment shaped like it rather than a local pandas stand-in. **Packaging**: a wheel plus `spark-submit`, a container image, or a declarative bundle that ships code and job definitions together. **Environment promotion**: separate dev, staging and production targets with the same code and *parameterised* configuration — cluster size, paths, secrets — rather than edited-per-environment code. **Deployment as code**: the job, schedule and cluster definitions live in version control next to the pipeline, applied by CI rather than clicked.

**Why** — because the alternative fails in a specific way: the code in Git and the job running in production drift apart, and nobody can say when they diverged or what the last known-good version was. Two practices carry most of the value. **Parameterise instead of branching per environment** — the same artefact promoted through environments is the only way a staging test tells you anything about production. And **test where it will run**: a transformation that passes against a local DataFrame can still fail on the cluster for reasons this path spends whole topics on — a UDF that is not serializable (**I18**), a null that only appears at scale, a config that differs (**I49**). The two mistakes worth naming: a test suite that builds a `SparkSession` per test, which makes the suite slow enough that CI stops running it (**A43**); and a deployment that has no rollback, where "fix forward" is the only option at 3am.

**Learn** — no book on this path covers it; the current guidance is vendor documentation and field write-ups rather than a text · docs: [Databricks CI/CD best practices](https://docs.databricks.com/aws/en/dev-tools/ci-cd/best-practices) — the clearest published statement of the separate-workspaces, parameterise, bundle-as-a-unit model, and the one to read even if you never touch Databricks; [`pyspark.testing`](https://spark.apache.org/docs/latest/api/python/reference/pyspark.testing.html) for the assertion helpers; the [AWS Spark CI/CD pipeline construct](https://awslabs.github.io/data-solutions-framework-on-aws/docs/constructs/library/Processing/spark-cicd-pipeline) for a worked non-Databricks pipeline · related: **A43** (the tests this runs), **I49** (what `spark-submit` does with your configuration), **E47** (the orchestrator you are deploying into), **A40**, **E60**

**Milestone** — take one pipeline and put it through the whole loop: a wheel or image built in CI, unit tests plus one integration test that runs against a cluster-shaped environment, and a deploy step that applies the job definition from version control to a staging target. Promote the *same artefact* to a second environment with only parameters changed, and prove nothing was rebuilt. Then rehearse the failure: make a change that passes unit tests and breaks in staging, and say which test you were missing. Finally, describe your rollback in one sentence — if it is "revert the commit and redeploy", say how long that takes and whether the data written in between needs undoing.
...
flowchart LR
    ...
    subgraph ADV["Advanced · 63 · 97–134 hrs"]
...
The strands not shown on the diagram — ingestion depth (I28–I35), procedural SQL (I40–I43), formats and types (I44–I46), shared variables (I47), declared constraints (I48), submission (I49), the Python process (I50–I51), names and the 4.x SQL surface (I52–I53), reliability (A25–A28), the file boundary (A29–A31), pipelines (A39–A42), practice (A43–A45), pushdown and the write path (A46–A48), stragglers (A49), the connector API itself (A50–A53), machine learning (A44, A54–A57), shuffle mechanics (A58), arbitrary state (A59–A60), keeping a table maintained (A61), the streaming-first table format (A62), automating the lakehouse (A63), and most of Expert — are read **on demand**, when the underlying problem finds you. They are written to the same standard as the main line; they are simply not sequential coursework.

### Where you are

**Done:** the Beginner level and five Intermediate topics — **B1**–**B4**, **B6**–**B8**, **B10**–**B11**, and **I1**, **I8**, **I10**, **I16**, **I24**. That is **14 of 188**, with chapters written for each in [`docs/spark-book/`](spark-book/index.md).
...
**If you only do three things next.** Clear **I10** first: it is the one wrong-not-dated chapter whose subject the audits also extended, so the rewrite now has **I50** (the Python worker that runs your UDF) and **I51** (worker logging, the two profilers, `pyspark.errors`) to lean on rather than asserting a hierarchy. Then **I25**–**I26**, because everything in Advanced assumes you can read a plan — and **I26** now carries the 4.2.0 UI, which is not the UI any book describes. Then **B11**, which is cheap to clear and has grown two neighbours worth reading with it: **I52** (how a name resolves, `SET PATH`, `system.builtin` versus `system.session`) and **I53** (pipe syntax, session variables, `EXECUTE IMMEDIATE`, `QUALIFY`).

**What the audits added that no chapter covers yet.** Thirty-nine topics, and these are the ones a reader at your position meets soonest: **I48** table constraints and **I49** `spark-submit` in Intermediate; **A50**–**A53** the DSv2 connector interface, catalogs, connector functions and write requirements; **A54**–**A57** feature engineering, the algorithm inventory, ALS and model evaluation, which turned MLlib from one thin topic into a real strand; **A58** the shuffle bytes — two checksums, codecs, the transport; **A59**–**A60** `transformWithState` and reading a query's state back as a DataFrame; **E55**–**E60** dynamic allocation, decommissioning, the Kubernetes pod build, the Operator, distributed training, and operating Connect.

**What v2 adds that you have not seen at all:** **B5** (ANSI, promoted into Beginner because it now governs every cast you write), **I7** (geospatial — and read its rewritten form: 4.2.0 ships five `ST_*` functions and no distance function, which is not what a release note suggests), and **I12** (the pandas API on Spark). None had a topic in v1.

**A caveat about depth.** The 144 topics carried over from v1 average about a third the length of the 41 written during the audits, and they are thinner in the same way: scope stated, mechanism and defaults missing. Where a topic here reads as a gesture rather than an explanation, that is the reason — treat its **Learn** line as the real content and expect to go to the docs and the source yourself.
...
Every v1 topic appears exactly once. The table below lists the seven topics that were new when v2 was written; the forty-two added since carry a **New topic** line in place of a v1 code and are not repeated here — thirty-nine from the feature-history audits (**B12**, **I5**, **I6**, **I13**, **I14**, **I44**–**I53**, **A46**–**A60**, **E52**–**E60**) and **A61**–**A63** from the lakehouse-coverage review. The five out-of-run codes at the front of that list are the ones an audit inserted *before* the end of a level, back when this page still renumbered; everything after **I44** was appended, which is why the ranges look tidy and the first five do not.
...
- Local table-format checkouts — `C:\opt\learn\spark\repos\delta` (tag `v4.3.1`) behind **I37** and **A39**, `C:\opt\learn\spark\repos\iceberg` (tag `apache-iceberg-1.11.0`) behind **I38** and **A61**, `C:\opt\learn\spark\repos\paimon` (tag `release-2.0.0`) behind **A62**, and `C:\opt\learn\spark\repos\amoro` (tag `v0.8.1-incubating`) behind **A63**. Every default those six topics quote is read from these trees rather than from the projects' documentation, which states some of them loosely and others not at all — and the Amoro checkout is also where its version lag is visible, since the root `pom.xml` pins the Iceberg and Paimon versions it was built against
...
- `docs/sql-data-sources-v2.md` — added 2026-03-28 by SPARK-54509 and shipping first in 4.2.0, which is what lets **A50** cite a docs page at all; its section list also confirms what the SQL-syntax reference still lacks (`CALL`, `SHOW PROCEDURES`, `DELETE`, `UPDATE`, `MERGE INTO`)
