...
> **Created:** 2026-08-09 — first full re-carve of the path since it was written, and rebuilt since: 187 topics in strands, every capability area of the [feature history](reference/spark-feature-history/index.md) audited row by row against Spark 4.2.0. The three most recent passes are below; the full dated record of every pass — including the ones that corrected a wrong claim rather than adding a topic — is the [changelog](learning-path-v2-changelog.md).
>
...
flowchart TD
    ...
    A["<b>Advanced</b> — make it fast, make it stream<br/>62 topics · 97–134 hrs"]
...
**Estimated time:** 97–134 hrs · **62 topics**
...
flowchart LR
    ...
    subgraph ADV["Advanced · 62 · 97–134 hrs"]
...
Every v1 topic appears exactly once. The table below lists the seven topics that were new when v2 was written; the thirty-six added since — thirty-four from the feature-history audits (**I44**–**I53**, **A46**–**A60**, **E52**–**E60**) and **A61**–**A62** from the lakehouse-coverage review — carry a **New topic** line in place of a v1 code and are not repeated here.
...
- Local table-format checkouts — `C:\opt\learn\spark\repos\delta` (tag `v4.3.1`) behind **I37** and **A39**, `C:\opt\learn\spark\repos\iceberg` (tag `apache-iceberg-1.11.0`) behind **I38** and **A61**, and `C:\opt\learn\spark\repos\paimon` (tag `release-2.0.0`) behind **A62**. Every default those five topics quote is read from these trees rather than from the projects' documentation, which states some of them loosely and others not at all
