...
**Why** — because every one of those is really the same question — *how much state, for how long* — and the watermark is the only answer Spark has. Without one, state grows without bound and the job dies days later with an OOM that names none of this; with one set too tight, late events are dropped silently and your counts are quietly wrong. The asymmetry worth internalising: **a watermark is a promise about lateness, not a guarantee about completeness**. Output mode compounds it — append emits a window only once the watermark has passed it, so a tight watermark trades correctness for latency and a loose one trades latency for state. The 4.x additions do not change that trade; they give you finer control over it (**A59**'s per-variable TTL) and the ability to look at the state you are holding (**A60**).
...
**What the audits added that no chapter covers yet.** Forty-one topics, and these are the ones a reader at your position meets soonest: **I48** table constraints and **I49** `spark-submit` in Intermediate; **A50**–**A53** the DSv2 connector interface, catalogs, connector functions and write requirements; **A54**–**A57** feature engineering, the algorithm inventory, ALS and model evaluation, which turned MLlib from one thin topic into a real strand; **A58** the shuffle bytes — two checksums, codecs, the transport; **A59**–**A60** `transformWithState` and reading a query's state back as a DataFrame; **E55**–**E60** dynamic allocation, decommissioning, the Kubernetes pod build, the Operator, distributed training, and operating Connect.
...
Every v1 topic appears exactly once. The table below lists the seven topics that were new when v2 was written; the thirty-seven added since — thirty-four from the feature-history audits (**I44**–**I53**, **A46**–**A60**, **E52**–**E60**) and **A61**–**A63** from the lakehouse-coverage review — carry a **New topic** line in place of a v1 code and are not repeated here.
