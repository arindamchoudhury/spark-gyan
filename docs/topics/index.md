...
- **pandas UDFs and PyArrow serialisation** — Rioux Ch 9 only. Series-to-Series, Iterator of Series, group aggregate, group map; split-apply-combine; cold start; mapInPandas; Arrow-native UDFs (Spark 4.1). *(Add a second source to write this page.)*
- **split-apply-combine in PySpark** — Rioux Ch 9 only. groupby() + applyInPandas() vs groupby() + agg(); batch composition control; OOM risk on skewed groups. *(Add a second source to write this page.)*
- **Window functions** — Rioux Ch 10 only. WindowSpec builder; three function families (aggregate, ranking, analytic); row vs range frames; default frame behaviour; UDFs over windows. *(Add a second source to write this page.)*
- **Window frame boundaries** — Rioux Ch 10 only. rowsBetween vs rangeBetween; unbounded/growing/static windows; the ordered-window default frame surprise; time-series sliding windows with rangeBetween. *(Add a second source to write this page.)*
- **Ranking and analytic window functions** — Rioux Ch 10 only. rank/dense_rank/percent_rank/ntile/row_number comparison; lag/lead for time-series features; cume_dist. *(Add a second source to write this page.)*
